  ...
  <CommonPage>
    <div style="display:flex">
      <div>
        ...
        <n-button type="tertiary" @click="addArea">
          填充选中颜色
        </n-button>
        <n-button type="tertiary" @click="removeLastArea">
          删除最后一个区域
        </n-button>
      ...
      <div class="container">
        ...
        <div v-for="(row, rowIndex) in rows" :key="rowIndex" class="dot-row">
          <div
            v-for="(col, colIndex) in cols" :key="colIndex" class="dot-wrap" :class="{
...
function isWithinAnyArea(row, col) {
...
function addArea() {
  ...
  const newArea = [
    [Number.parseInt(startX.value), Number.parseInt(startY.value)],
    [Number.parseInt(endX.value), Number.parseInt(endY.value)],
  ]
